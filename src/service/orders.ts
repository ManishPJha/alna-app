...
export type OrderStatus = 'DRAFT' | 'RECEIVED' | 'PREPARING' | 'READY' | 'SERVED' | 'CANCELLED';
...
export const orderService = {
  ...
  getByRestaurant: (restaurantId: string, filters?: Omit<OrderFilters, 'restaurantId'>) => {
    const params = {
      restaurantId,
  ...
  bulkUpdateStatus: (orderIds: string[], status: OrderStatus) =>
    apiClient.patch<{ updated: number }>('/orders/bulk', { orderIds, status }),

  // Export orders (for reporting)
  export: (restaurantId: string, format: 'csv' | 'json', filters?: OrderFilters) => {
