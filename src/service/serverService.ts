...
async function apiCall<T>(
    ...
    try {
      ...
      const response = await fetch( params ? `${baseURL}/${path}/${params}` : `${baseURL}/${path}`, config);
...
export async function getRequest(path: string) {
  ...
  return apiCall(`${path}`, { cache: "no-store", headers: { Cookie: cookieStore.toString() } })
}

export async function getRequestWithRevalidate(path: string) {  
  const cookieStore= await cookies()  
  return apiCall(`${path}`, { next: {revalidate: 360}, headers: { Cookie: cookieStore.toString() } })
}

export async function getApiWithParam(path: string, param: string) {
  return apiCall(`${path}/${param}`, { next: { revalidate: 3600 }, headers: { Cookie: cookies().toString() } })
}
