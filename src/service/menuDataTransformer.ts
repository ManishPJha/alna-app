...
export class MenuDataTransformer {
    ...
    static fromApiResponse(apiResponse: any): MenuFormData | null {
        ...
        try {
            ...
            const menuItems = this.extractMenuItems(menuData, category);

            console.log('Extracted data:', { restaurant, category, menuItems });
    ...
    private static extractCategoryInfo(data: any): any | null {
        ...
        const possiblePaths = [data.category, data.menu?.category];
     ...
     */
    private static extractMenuItems(data: any, category: any): any[] {
        const items: any[] = [];

        // Try to get items from category.menuItems
        if (category && data.category?.menuItems) {
            return data.category.menuItems.map((item: any) =>
                this.transformMenuItem(item)
            );
        }

        // Try to get items from menu.category.menuItems
        if (data.menu?.category?.menuItems) {
            return data.menu.category.menuItems.map((item: any) =>
                this.transformMenuItem(item)
            );
        }
