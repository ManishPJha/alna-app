  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/ask-ai', {
        ...
        body: JSON.stringify({ restaurantId, question: input, includeFaq }),
      });
      const data = await res.json();
      const answer = data.answer ?? '⚠️ Sorry, I could not find an answer.';
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50">
      <div className="w-full max-w-md h-[80vh] bg-gray-900 text-white rounded-xl shadow-lg flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-800">
          <div>
            <h2 className="text-lg font-semibold">Ask about the menu</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.length === 0 && (
            <div className="text-center text-gray-500 text-sm mt-10">
              Example: <span className="italic">"What are today’s specials?"</span> or{" "}
              <span className="italic">"Show me vegan options."</span>
            </div>
          )}
          {messages.map((msg, i) => (
            <div
